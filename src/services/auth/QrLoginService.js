...
export class QrLoginService {
    ...
    async qrLoginV2ForSecure(nonce) {
        try {
            ...
            if (this.qrPrivateKey && loginResult) {
                loginResult.e2eeKey = await this.processQrE2EEInfo(loginResult);
            }
    ...
    async processQrE2EEInfo(loginResult) {
        try {
            ...
            const e2eeInfo = loginResult.metaData || loginResult[10];
            if (e2eeInfo && this.qrPrivateKey) {
                ...
                if (decodedKey) {
                    ...
                    Logger.warn('QR_LOGIN', 'E2EE key decoding failed');
                }
            } else if (e2eeInfo) {
                Logger.warn('QR_LOGIN', 'E2EE info found but no QR private key available');
            } else {
                Logger.warn('QR_LOGIN', 'QR response did not include an E2EE key chain');
            }
    ...
    decryptKeyChain(publicKey, privateKey, encryptedKeyChain) {
        try {
            ...
            throw new Error(`QR E2EE key-chain decryption failed: ${error.message}`);
        }
    }
    ...
    async saveQrE2EEKeysToStorage(loginResult) {
        return this.processQrE2EEInfo(loginResult);
    }
    
    async loadCertificate() {
        try {
            const sessionCertificate = this.sessionStore?.load()?.certificate;
            if (sessionCertificate) return sessionCertificate;
            if (this.storageManager && this.storageManager.initialized) {
                const certificate = this.storageManager.loadData('certificate');
                if (certificate) {
                    Logger.info('QR_LOGIN', 'Certificate loaded from session storage');
                    return certificate;
                } else {
            ...
            if (fs.existsSync(certPath)) {
                ...
                Logger.info('QR_LOGIN', 'Certificate loaded from legacy storage');
                return certificate;
            } else {
    ...
    async saveCertificate(certificate) {
        try {
            ...
            if (this.sessionStore) {
                this.sessionStore.save({ certificate });
                return true;
            }
            ...
            if (this.storageManager && this.storageManager.initialized) {
                ...
                if (saved) {
                    Logger.success('QR_LOGIN', 'Certificate saved to session storage');
                    return true;
                } else {
    ...
    async *qrLoginFlow() {
        try {
            ...
            yield { step: 'session', data: session, message: 'Session created' };
